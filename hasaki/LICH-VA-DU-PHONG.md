...
| **5S Canh B11** (thêm 24/09/2026) | mỗi **giờ** :35, cửa **07:00–18:00 T2–T7** đặt ở đầu main (tick ngoài giờ 0 lượt gọi) | `canh-b11.mjs` — 3 luật trên 1 GET board WF591/lượt: ① B1.1 treo >48h → bình luận TAG QLTT (ô QLBP02 của B1, mention `@[Tên](user_id:…)`) nhắc NV, CHỈ khi NV thuộc Phát triển cửa hàng/Đóng gói kho 170 (loc 398), mỗi task nhắc 1 lần (sổ `.canh-b11.json` + marker); ② B1 ≥2 NV → B1.1 gán đủ người + `type=3` (nhóm · mỗi thành viên; push-5s cũng tự đặt lúc tạo); ③ B1.1 xong ĐỦ TỪNG thành viên → điền `NVVP04` (Nhân viên vi phạm, mã phẩy) vào B3.1 rồi CHỈ LƯU, không đóng bước | `canh-b11.log` |
