 ...
 * ─────────────────────────────────────────────────────────────────────────────────────────────── */
const TRAN_DONG_NGUON = 15;      // số dòng chi tiết tối đa trong báo cáo (còn lại gộp thành 1 câu)
...
function dongNguon(sku, rows) {
  ...
  return `- ${sku} (${soVN(tong)}${uom} · ${rows.length} UID) ← ${nguon.join(" ; ")}`;
}
...
async function bcF0A0(t, cfg) {
  ...
  const chiTiet = coNguon.slice(0, TRAN_DONG_NGUON).map((x) => dongNguon(String(x.sku), ng.get(String(x.sku))));
  const conLai = coNguon.length - chiTiet.length;
  ...
  const dong = [
    ...
    Object.keys(dem).length
      ...
      : "",
    chiTiet.length ? "Chi tiết từng mã (số lượng · UID ← phiếu đưa vào bin):" : "",
    ...chiTiet,
...
const SO_TAY = [
  ...
  /* NGOÀI PHẠM VI BOT — ĐÚNG MỘT task, chủ máy chốt lại 20/08/2026: task tên CHÍNH XÁC "Sắp xếp
     ...
     Vì sao khoá bằng CẢ TÊN NEO HAI ĐẦU (`^.../**
 * ============================================================================
 *  task-hangngay.mjs — 9 TASK HẰNG NGÀY trên work.hasaki.vn: soi + nộp báo cáo
 * ============================================================================
 *  Mỗi sáng hệ thống bắn cho tamlc@hasaki.vn một BỘ TASK CỐ ĐỊNH (T2–T7):
...
const SO_TAY = [
  ...
  { khop: /bất thường.*MATERIAL - MTG/i, nhom: "A", dung: (t) => bcF0A0(t, { company: "1002", warehouse: "1177", ten: "MTG" }) },
  { khop: /bất thường.*MATERIAL - GARMENT/i, nhom: "A", dung: (t) => bcF0A0(t, { company: "1005", warehouse: "1339", ten: "GARMENT" }) },
) LẪN NGƯỜI GIAO: khớp lỏng kiểu `/tại kho tổng/`
     sẽ ăn lây bất kỳ task nào chứa cụm đó (ví dụ "Sắp xếp hàng hóa tại kho tổng ca 2" của người
     khác) và im lặng không nộp. Lệch một trong hai điều kiện ⇒ rơi vào nhánh "task LẠ": bot KÊU TO
     và không nộp — hướng sai an toàn, không bao giờ nộp hộ task của người ta.
...
/* Khớp theo TÊN, và với mục nào khai `nguoiGiao` thì phải đúng luôn người tạo task (`created_by`).
   Truyền `t` vào là tuỳ chọn — thiếu `t` thì mục có `nguoiGiao` không khớp, tức là an toàn. */
 ...
 *  kho, #13373905 Dán tem QC Fail) bị lặng lẽ bỏ lại — log chỉ ghi "nộp 6 · bỏ qua 4", không nói
