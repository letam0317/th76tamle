...
| File | Vai trò |
|---|---|
| `auto-export-sync.js` | **Bộ đồng bộ chính — TỰ ĐỘNG 100%** (không cần bấm nút, không cần file Downloads). Lấy token → `POST /api/hr/excel-io/export` (queue, chia cửa sổ ≤60 ngày vì hạn 3 tháng/lần) → poll `GET /api/hr/excel-io` tới khi `status=1` → **tải file công khai** `wshr.hasaki.vn/production/hr/<file_path>` → đổi path ảnh → URL `hr-media` → gộp+khử trùng theo Task Code → ghi `5S-TASKS`. |
...
| `5S Dong bo dashboard` | mỗi 60 phút | `auto-export-hidden.vbs` | **TỰ ĐỘNG 100%**: tự xuất qua API → tải file → đẩy `5S-TASKS` (Module E) |
