...
| `DONG-BO-TASK-AN.vbs` / `day-bo-task-hidden.vbs` | Chạy `DONG-BO-TASK.bat` **hoàn toàn ẩn**; `day-bo-task-hidden.vbs` là bản Task Scheduler gọi theo lịch. |
...
**Quy trình HYBRID (bán tự động):** bạn bấm nút **Export** trên workflow 591 → trình duyệt tải file về Downloads →
lịch **`5S Dong bo dashboard`** (chạy ngầm mỗi 30') tự đọc file mới nhất và đẩy lên `5S-TASKS`. Có
`.last-sync.json` để **bỏ qua nếu chưa có file export mới** (không ghi đè thừa).

> ⚠️ **Vì sao không tự động 100%?** API board (`detail-workflow-task`) không mang đủ trường 5S ổn định;
> còn file Export sinh qua job async (`POST /api/hr/excel-io/export`, tối đa **3 tháng/lần**) nhưng khâu
> **tải file** bị chặn (hr-media 403, CORS, endpoint download ẩn) nên chưa dựng lại được headless. Do đó
> giữ bước bấm "Xuất" thủ công, tự động hoá nửa còn lại.
...
| `5S Dong bo dashboard` | mỗi 30 phút | `day-bo-task-hidden.vbs` | Đọc file Export mới trong Downloads → đẩy `5S-TASKS` (Module E) |
